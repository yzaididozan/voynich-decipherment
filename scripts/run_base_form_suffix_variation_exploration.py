#!/usr/bin/env python3
"""Run Phase 3A.3 base-form-preserving suffix variation exploration.

Intended repository destination:
    scripts/run_base_form_suffix_variation_exploration.py

Grid:
    4 languages x K={1,2,4} x 5 seeds = 60 controls
...
from src.ciphers import base_form_suffix_variation
...
def verify_master_key_invariance(
    ...
    *,
    ...
    """Ensure each source/seed uses one identical base/suffix key across K."""
    groups = {}
    ...
    for (source_id, seed), rows in groups.items():
        ...
        if len(fingerprints) != 1:
            raise ValueError(
                f"{source_id}/seed={seed}: base/suffix master key changed across K"
            )
...
def require_level2_compatibility(
    ...
    if (
        ...
        != list(phase3a3["operational_relations"])
    ):
        raise ValueError(
            "Phase-3A.3 operational relations differ from Level 2"
        )

    for source_id, source in phase3a3["sources"].items():
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Generate and evaluate Phase-3A.3 base-form-preserving suffix "
            "variation controls on frozen Level-2 TRAIN/VALIDATION."
        )
    )
    parser.add_argument(
        "--config",
    ...
    try:
        ...
        if (
            ...
            != "base-form-suffix-variation-exploration-v1"
        ):
            raise ValueError(
                "Unexpected Phase-3A.3 experiment_id"
            )
        if config.get("status") != "exploratory":
            raise ValueError(
                "Phase 3A.3 must remain explicitly exploratory"
            )
        ...
        if variant_counts != [1, 2, 4]:
            raise ValueError(
                "Suffix K grid must be exactly [1,2,4]"
            )
        if int(mechanism["master_suffix_count"]) != 4:
            raise ValueError(
                "master_suffix_count must be exactly 4"
            )
        if (
            mechanism["coverage"]
            ...
            raise ValueError(
                "Phase 3A.3 coverage must be all TRAIN/VALIDATION tokens"
            )
        ...
        if len(seeds) != 5:
            raise ValueError(
                "Phase 3A.3 requires exactly five frozen seeds"
            )
        ...
        generator_hash = sha256_file(
            Path(base_form_suffix_variation.__file__)
        )
        ...
        print("=" * 96)
        print(
            "VOYAGER PHASE 3A.3 — BASE-FORM-PRESERVING SURFACE VARIATION"
        )
        print("=" * 96)
            ...
            for k in variant_counts:
                for seed in seeds:
                    ...
                    if (
                        ...
                        required = {
                            "phase3a3_config_sha256": config_hash,
                        ...
                        provenance = {
                            "phase3a3_config_path": str(
                                args.config
                            ),
                            "phase3a3_config_sha256": config_hash,
                            "generator_path": str(
                                Path(
                                    base_form_suffix_variation.__file__
                                )
                            ),
                            "generator_sha256": generator_hash,
                            "source_path": str(
                    ...
                    if (
                        ...
                        for key_name, value in required.items():
                            if saved.get(key_name) != value:
                                raise ValueError(
                                    f"Existing Phase-3A.3 result has "
                        ...
                        for row in relation_rows:
                            row["variant_count"] = k
                            row["phase3a3_mechanism"] = (
                                "base_form_suffix_variation"
                            )
                        ...
                        result_summary = {
                            ...
                            "interpretation": (
                                "Exploratory base-form-preserving suffix result. "
        ...
        top_summary = {
            ...
            "next_step": (
                "Interpret the K trajectory jointly across all five relations. "
                "If a stable candidate region emerges, write and freeze a "
                "separate Phase-3B confirmatory protocol before any reserved-"
                "test use. Otherwise retain the negative result and design the "
                "next exploratory intervention without adapting to a reserved test."
            ),
        }
        ...
        print("=" * 96)
        print("PHASE 3A.3 EXPLORATION COMPLETE")
        print("=" * 96)
        ...
        print(
            ...
            "Phase 3A.3 is exploratory."
        )
        return 0

    except KeyboardInterrupt:
        print(
            "\nPHASE-3A.3 INTERRUPTED. Existing hash-matched controls/results "
        ...
        print(
            f"PHASE-3A.3 ERROR: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 1
